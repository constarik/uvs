...
Participant ids **MUST** be unique within a draw. A record containing duplicate ids is **invalid**, and a verifier **MUST** reject it rather than rank it — with duplicates, the `id` tie-break no longer yields a total order, and two entries would collide on the same score. An **empty** id is likewise **invalid**: a record containing one **MUST** be rejected, not ranked.
...
`randomness` **MUST** be taken as the SHA-256 of the round's signature bytes (the beacon's own `randomness` field), and any verifier **MUST** be able to re-fetch the named round and confirm it. The signature bytes **MUST** be decoded from the relay's documented encoding (hex on the quicknet HTTP API) before hashing; a verifier **MUST** reject a round whose signature is absent, malformed, or not the chain's expected length — a malformed beacon response is not a usable randomness source. `roundAt(t)` and `timeOfRound(r)` **MUST** be computed in exact integer arithmetic (floor division; the beacon constants above are integers), so round selection is byte-identical across languages (core §3).
...
### 5.5 Binding the prize rules to the point of sale (OPTIONAL — input-integrity axis)

§5.4 proves the prize pool existed before the *randomness*. It does **not** prove that the pool shown to a buyer at the **moment of sale** is the pool the draw later used: an operator could advertise one prize, then attest and draw another. By §8 this is an **input-honesty** question, *separate from the outcome tier* — this section narrows it but does **not** change a draw's 🔴/🟡/🟢 classification.

**Two-phase commitment.** The two inputs have different lifetimes — the participant list closes late (tickets sell until the deadline), the rules are fixed early — so they are committed in two phases:

- **Phase 0 — before sales open.** The operator fixes `{ drawId, prizePool }` and obtains a **rules attestation** (below). The attestation's time **MUST** precede the opening of sales, proven by operator-independent evidence (RFC 3161 or drand-time, per §5.4). The selection procedure itself is not a per-draw field: it is fixed by the standard (§3) and pinned by `uvsVersion` (§7). The only per-draw resolution choice — the §6.1 rounding `mode` — already lives inside `prizePool`, so it is attested by committing `prizePool` and nothing further need be named here.
- **Phase 1 — at the draw.** The final `participants[]` are committed and the **unchanged** Phase-0 `prizePool` is reused; the outcome is bound per §5.2 and proven per §5.4.

**`rulesAttested(drawId, prizePool)`** is satisfied by **either** of the following — interchangeable, the operator chooses by trust/cost profile:

- **Signature.** A neutral-registry key (core §10.1) signs `{ drawId, CanonicalJSON(prizePool) }`. Verifiable offline against the published key; the proof travels with the ticket and does not depend on the registry's current state. Cost: one long-lived signing key (rotation + a published fingerprint).
- **Stamp.** `{ drawId, prizePool }` is published on the neutral registry and timestamped (RFC 3161 or an append-only medium). No standing secret; trust rests on the published record.

In **both** forms: the attestation time **MUST** be before sales open, and a lookup of `drawId` on the neutral registry **MUST** return exactly this `prizePool`.

**Ticket binding.** Every ticket or receipt **MUST** carry `drawId` and a reference to the neutral record as **both** a human-readable URL (`<neutral-host>/<drawId>`) **and** a QR encoding the same URL. The buyer reaches the record through a channel the operator does not control; the QR is convenience, not the trust anchor.

#### 5.5.1 What this does not close (display & channel residuals)

Two gaps remain that **no cryptographic step removes**, and a conforming implementation **MUST NOT** imply otherwise:

- **Display residual.** The operator controls the purchase screen and may render a prize list that differs from the attested one, beside a correct `drawId`. The attestation binds the *record*, not the pixels shown to the buyer.
- **Channel residual.** The operator prints the ticket and may substitute a look-alike URL/QR pointing at a spoofed host. The binding holds only if the buyer reaches the **genuine** neutral record.

Both reduce to one fact: cryptography makes the true rules neutral and recomputable, but cannot force a buyer to reach them. The reachable goal is therefore not prevention but **converting a private substitution into publicly provable fraud** — the same posture as the rest of the standard (§12).

Three controls narrow the residuals; all are operational or legal, **not** cryptographic:

1. **Single well-known anchor.** The canonical lookup lives at **one** operator-independent host, reached by reputation (as a result is checked at an official site, not via a stranger's link). One canonical host makes any other URL a detectable claim.
2. **Contractual warranty.** The draw agreement obligates the operator to print the genuine URL+QR and to display the attested `prizePool`; a mismatch is a breach (liability, termination, public delisting). This is the *consequence* layer.
3. **Neutral test-entry (canary), OPTIONAL.** A spot-check that purchases through the operator's own flow and verifies the displayed pool, the printed URL/QR, and the presence of its own ticket in the committed set. **Known** (a cooperative audit right) suits an honest operator proving good faith; **covert and indistinguishable** (random account, timing, payment) is required to deter a hostile one, at probabilistic coverage. The test-entry is what makes the warranty *enforceable* by detecting a breach; it is detection, not prevention.
...
### 6.1 Participant-dependent pools (proportional rules)

A tier **MAY** declare its winner count as a deterministic function of the committed participant count `M` instead of an absolute integer. This lets "1 in 10 wins" or "top 5%" scale with turnout while staying fully reproducible.

A proportional tier declares a rate as an **integer numerator over an integer denominator** — never a floating-point percentage. A float such as `0.006` is not exactly representable and can flip a boundary comparison between languages, breaking byte-identical reproduction (core §3). Ratio "1 in K" is `num=1, den=K`; "5%" is `num=5, den=100`; finer rates use a larger denominator (`num=1, den=1000000`).

The resolved count is computed in **integer arithmetic** by one declared rounding mode (`\` is integer division):

```
floor         : count = (M*num)           \ den
round-half-up : count = (2*M*num + den)    \ (2*den)     # DEFAULT
ceil          : count = (M*num + den - 1)  \ den
```

All operands are non-negative, so integer division is floor — there is no truncation-direction ambiguity. An implementation **MUST** use integer types wide enough that `M*num` and `2*M*num + den` never overflow (64-bit suffices for `M`, `num` ≤ 10⁹; use arbitrary-precision integers beyond that), so a language with fixed-width integers reproduces the same count as one with unbounded integers (core §3). Numeric fields (`count`, `num`, `den`) **MUST** be JSON integers; a string or otherwise coerced value (e.g. `"5"`) **MUST** invalidate the record — no type coercion is permitted (core §5). The mode **MUST** be declared in the rule and is part of the committed record; a `rule` with `mode` absent is resolved as `round-half-up` **by definition**, so reproducibility never rests on an undeclared fallback. The `rule` object is hashed as part of `prizePool` under Canonical JSON (core §5). **round-half-up is the default**, because it is faithful to the stated odds: the expected number of winners is `M*num/den`, and rounding to the nearest integer represents it without bias. `floor` systematically *under-awards* — it discards the fractional expected winner, so "1 in 10" at `M=9` yields `0`; `ceil` guarantees at least one winner whenever `M ≥ 1`. Worked example, "1 in 10", round-half-up: `M=9 → 1`, `M=4 → 0`, `M=15 → 2`.
...
**Degenerate counts.** A tier whose resolved count is `0` (e.g. "1 in 10" with `M=4`) has no winners; this is valid and **MUST** be reproduced, not rounded up to one.

A proportional pool **MUST** publish, in the record (§7), both the **rule** (`num`, `den`, `mode`) and the **resolved counts** at the committed `M`. A verifier recomputes the counts from the rule and `M` and **MUST** reject any record whose stated counts disagree. Anti-grinding is unaffected: the rule and `M` are fixed at commit (`M` is the final committed participant list), so the round still publishes after the inputs are frozen.

> A proportional rule is also *more transparent* than a bare absolute count: an entrant reads "1 in 10" and checks that the winner count equals the rule applied to `M`, instead of trusting an arbitrary `count` the operator typed.
...
4. **Re-derivation** — confirm `participants[]` contains no duplicate ids (§3.1), run §3 over the committed list, and confirm the winners match what was announced. For any **proportional** tier (§6.1), recompute its resolved `count` from the declared `rule` and `M` and confirm it matches the record.
