...
> **Status: LIVE.** Pull and recompute at [uvs.uncloned.work/gacha](https://uvs.uncloned.work/gacha) — in-browser 🟡, neutral commit-reveal 🟡 (notarized at ×2 RFC-3161), and a drand-bound 🟢 batch on the registrar (`/gacha/commit`, `/gacha/reveal`, `/gacha/commit-batch`, plus public `GET /gacha/:sessionId` to fetch + replay a record by id, the analogue of `/draws/:id`). Sessions persist in Firestore (durable). The four reference resolvers reproduce the live server's records byte-for-byte.
